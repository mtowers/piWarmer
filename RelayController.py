...
class RelayController(object):
    ...
    def __init__(self, configuration, logger):
        ...
        self.ser = self.initialize_modem()

        if self.ser is None:
            self.log_warning_message("Serial Device not connected, quiting.")
            exit()

        self.fona = Fona(name="fona", ser=self.ser,
        ...
        for phone_number in self.configuration.allowed_phone_numbers:
            ...
            self.log_info_message("Sent starting message to " + phone_number)
        ...
        if self.configuration.is_mq2_enabled:
            gas_status = self.get_mq2_status()
    ...
    def send_message_to_all_numbers(self, message):
        ...
        if(self.configuration.push_notification_number not in self.configuration.allowed_phone_numbers)
            self.fona.send_message('"+' + self.configuration.push_notification_number + '"', message)
    ...
    def handle_on_request(self, status, phone_number):
        ...
        if phone_number is None:
            return CommandResponse(False, False, "Phone number was empty.")

        self.log_info_message("Received ON request from " + phone_number)

        if status == "1":
            return CommandResponse(False, False, "Heater is already ON")

        if self.is_gas_detected():
            return CommandResponse(False, True, "Gas warning. Not turning heater on")

        return CommandResponse(True, False,
                               "Heater turning on for " + str(self.configuration.max_minutes_to_run) + " minutes.")
    ...
    def process_message(self, message, phone_number=False):
        ...
        if "on" in message:
            ...
            message_response = actions.message
    ...
    def monitor_gas_sensor(self):
        ...
        while True:
            ...
            if detected and self.heater_relay.get_status() == "1":
                ...
                self.fona.send_message(self.push_notification_number, "GAS DETECTED")
                time.sleep(60)
            else:
